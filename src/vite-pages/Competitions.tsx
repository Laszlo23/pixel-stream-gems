...
const Competitions = () => {
  ...
  const creators = creatorPeriod === "week" ? TOP_TIPPED_CREATORS_WEEK : TOP_TIPPED_CREATORS_MONTH;

  return (
    <div className="min-h-screen bg-background hero-mesh">
      <Navbar />
      <main className="container mx-auto px-4 pt-20 pb-20 max-w-5xl space-y-8">
        ...
        <Tabs defaultValue="fans" className="w-full">
          ...
          <TabsContent value="fans" className="mt-6 space-y-4">
            <RewardsCard title="Weekly rewards" items={[...COMPETITION_REWARDS.tippingFans]} />
          ...
          <TabsContent value="creators" className="mt-6 space-y-4">
            <RewardsCard title="Top creator perks" items={[...COMPETITION_REWARDS.tippedCreators]} />
                  ...
                  <TableBody>
                    {creators.map((r) => (
                      <TableRow key={r.id}>
                        <TableCell className="text-muted-foreground">{r.rank}</TableCell>
                        <TableCell>
                          <Link to={`/u/${r.id}`} className="font-medium text-foreground hover:text-primary">
                            {r.name}
                          </Link>
                        </TableCell>
          ...
          <TabsContent value="lp" className="mt-6 space-y-4">
            <RewardsCard title="LP competition rewards" items={[...COMPETITION_REWARDS.lp]} />
            <Card className="rounded-2xl border-border/80 bg-card/70 backdrop-blur-sm overflow-hidden">
              <CardHeader>
              ...
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {TOP_LP_LEADERBOARD.map((r) => (
                      <TableRow key={r.wallet}>
                        <TableCell className="text-muted-foreground">{r.rank}</TableCell>
                        <TableCell className="font-mono text-xs">{r.wallet}</TableCell>
                        <TableCell>
                          <Link to={`/u/${r.creatorId}`} className="text-primary hover:underline text-sm">
                            {r.pool}
                          </Link>
                        </TableCell>
          ...
          <TabsContent value="flow" className="mt-6 space-y-4">
            <RewardsCard title="Supporter league rewards" items={[...COMPETITION_REWARDS.superfluid]} icon={<Crown className="w-4 h-4" />} />
            <Card className="rounded-2xl border-border/80 bg-card/70 backdrop-blur-sm overflow-hidden">
              <CardHeader>
